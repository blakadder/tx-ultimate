    ...
    import string
      if size(msg) > 0
        ...
        if msg[0..1] == self.header
          print("TXU: Message", msg, msg[4], msg[5])
          var lst = self.split_55(msg)
          for i:0..size(lst)-1
            msg = lst[i]
            var event = ""
            var params = ""
            if msg[2] == 0x01 && msg[3] == 0x02 # 02 signifies a touch event
              print('Message Type:', msg[4], msg[5])
              if (msg[4] == 0x02 && msg[5] != 0) || msg[4] == 0x01 # 2 is a press and 1 is release
                if   msg[5] < 0x0B 
                  event= "Short"
              ...
              if event != ""
                ...
                jm = string.format("{\"TXUltimate\":{\"Action\":\"Touch\",\"Event\":\"%s\"}}",event)
                tasmota.publish_result(jm, "RESULT")
              end
            end
          end
